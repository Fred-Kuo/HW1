...
{   unsigned long sum;
    ...
    while (value / 10)											// 不只是單一的個位數字
    {
        ...
        while (value > 0)
        {
           sum += (value % 10) ;                                // 加入最右側的一個數字(總合加1位)
           value /= 10         ;                                // 去掉最右側的一個數字(少1位)
             																		              																		// 去掉最右側的一個數字
        }   // end inner-while
        value = sum;											// 本回合的加總數字
    }   // end outer-while
...
{   unsigned long   M;
    ...
    do {
        ...
        cin >> inputS;
        isNum = true;
        for (int i = 0; i < inputS.length(); i++)              // i小於字串長度，檢查字串每個字元
            if (inputS[i] < '0' || inputS[i] >'9')			    // 在'0'~'9'範圍之外就是非數字
                isNum = false;
        if (!isNum)                                             //if 非數字(false) 跳出迴圈
            continue;
        ...
        {   if (level < N)															// 尚未達到指定長度
            ...
            {   bool	backtrack;
                ...
                {   level--;
                    ...
                    for (int i = ans[level] + 1; i < N; i++)              			// 選擇目前數字之後的可用數字(i=N繼續後退)
                        if (!used[i])												// 只限尚未放入ans的數字(false)
                        {
	                        ans[level] = i;                                         // ans填入目前數字+1
	                        used[i] = true;                                         // 此數字已使用
	                        level++;                                                // 往後一個數字(回到前一層迴圈繼續做)
